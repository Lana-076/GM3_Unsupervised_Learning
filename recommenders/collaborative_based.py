...
movies1 = title_list[14930:15200]
movies2 = title_list[25055:25255]
movies3 = title_list[21100:21200]
Moviez = movies1 + movies2 + movies3
edsa_movies = movies_df[movies_df['title'].isin(Moviez)]
...
model=pickle.load(open('resources/models/model', 'rb'))
...
def collab_model(movie_list,top_n):
    ...
    user = 200000
    p = 20
    # Gather the p users which are most similar to the reference user 
    sim_users = user_sim_df.sort_values(by=user, ascending=False).index[1:p+1]
    favorite_user_items = [] 
    common_favorites = {} 
    ...
    for item_collection in range(len(favorite_user_items)):
        for item in favorite_user_items[item_collection]:
            if item in common_favorites:
                common_favorites[item] += 1
            else:
                common_favorites[item] = 1

    # Sort the overall most popular items and return the top-N instances
    sorted_list = sorted(common_favorites.items(), key=operator.itemgetter(1), reverse=True)[:top_n]
